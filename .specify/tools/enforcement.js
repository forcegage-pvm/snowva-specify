...
class EnforcementMaster {
  ...
  async start() {
    ...
    // Layer 1: Process-Level Enforcement (separate process)
    console.log("🔧 LAYER 1: Process-Level Evidence Enforcement");
    try {
      console.log("🚀 Starting process enforcement in separate process...");

      // Start process enforcer in background
      this.processes.processEnforcer = spawn(
        "node",
        [path.join(this.toolsDir, "dev-enforcement.js"), "start"],
        {
          detached: true,
      ...
      this.processes.processEnforcer.stdout.on("data", (data) => {
        console.log("   🔧 ", data.toString().trim());
      });

      this.processes.processEnforcer.stderr.on("data", (data) => {
        console.log("   ⚠️ ", data.toString().trim());
      });

      console.log(
        "✅ Layer 1 ACTIVE - Process enforcement running in background"
      );
    } catch (error) {
    ...
    // Layer 2: Directory Watcher (separate process)
    console.log("👁️  LAYER 2: Real-Time Directory Monitoring");
    try {
      console.log("🚀 Starting directory watcher in separate process...");

      // Start directory watcher in background
      this.processes.directoryWatcher = spawn(
        "node",
        [path.join(this.toolsDir, "evidence-watcher.js"), "start"],
        {
          detached: true,
          stdio: ["ignore", "pipe", "pipe"],
        }
  ...
  stop() {
    ...
    // Stop Layer 1 (Process Enforcer)
    if (this.processes.processEnforcer) {
      try {
        this.processes.processEnforcer.kill("SIGTERM");
        console.log("   Process enforcement stopped");
      } catch (error) {
        console.log("⚠️  Could not stop process enforcer:", error.message);
      }
    }

    // Stop Layer 2 (Directory Watcher)
    if (this.processes.directoryWatcher) {
      try {
        this.processes.directoryWatcher.kill("SIGTERM");
        console.log("   Directory monitoring stopped");
      } catch (error) {
        console.log("⚠️  Could not stop directory watcher:", error.message);
      }
    }
  ...
  getStatus() {
    ...
    try {
      ...
      console.log(
        "   Status: 🟢 Available (activate with enforcement.js start)"
      );
    } catch (error) {
  ...
  async test() {
    ...
    try {
      ...
      const testProcess = spawn(
        "node",
        [path.join(this.toolsDir, "dev-enforcement.js"), "test"],
        {
          stdio: "inherit",
        }
      );
    ...
    console.log(
      "📊 Test Summary: Constitutional enforcement components verified"
    );
  }
}
