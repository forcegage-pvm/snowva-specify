...
const {
  ConstitutionalEnforcementActivator,
} = require("./activate-enforcement.js");
...
async function testEnforcement() {
  ...
  try {
    const fakeEvidencePath = path.join(
      process.cwd(),
      "evidence/TEST/fake-process-test.json"
    );
    ...
    console.log(
      "   ❌ LAYER 1 FAILED - File was created (enforcement not working)"
    );
    ...
    if (error.message && error.message.includes("fabrication blocked")) {
      console.log(
        "   ✅ LAYER 1 SUCCESS - Process-level enforcement blocked fabrication"
      );
  ...
  try {
    ...
    const evidencePath = path.join(
      process.cwd(),
      "evidence/TEST/fake-watcher-test.json"
    );
    ...
    if (fs.existsSync(evidencePath)) {
      ...
      console.log(
        "   ✅ LAYER 2 SUCCESS - Directory watcher deleted fabricated file"
      );
    }
  } catch (error) {
  ...
  try {
    const legitPath = path.join(
      process.cwd(),
      "evidence/TEST/legitimate-test.md"
    );
...
`;
    ...
    if (fs.existsSync(legitPath)) {
      console.log(
        "   ✅ LEGITIMATE EVIDENCE SUCCESS - File created and preserved"
      );
      ...
      console.log(
        "   ❌ LEGITIMATE EVIDENCE FAILED - File was incorrectly blocked"
      );
    }
  } catch (error) {
  ...
  console.log("============================");
  console.log(
    "All layers tested - Constitutional enforcement verification complete"
  );
  console.log("");
