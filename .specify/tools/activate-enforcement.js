...
const { ProcessLevelEvidenceEnforcer } = require('./process-level-evidence-enforcer.js');
...
class ConstitutionalEnforcementActivator {
  ...
  async activate(options = {}) {
    ...
    if (layers.includes(2)) {
      ...
      try {
        this.directoryWatcher = new EvidenceDirectoryWatcher(this.workspaceRoot);
    ...
    if (layers.includes(3)) {
      ...
      const hookPath = path.join(this.workspaceRoot, '.specify/hooks/pre-commit');
