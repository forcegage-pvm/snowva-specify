...
export async function GET(request: NextRequest) {
  try {
    ...
    const { page, limit, status, customerId, search, sortBy, sortOrder } =
      validation.data;

    const filters = {
      page,
      pageSize: limit,
      sort: (sortBy || "createdAt") as
        | "createdAt"
        | "updatedAt"
        | "totalAmount"
        | "quoteNumber"
    ...
    try {
      ...
      const result = {
        ...
        pagination: {
          ...
          total: filteredQuotes.length, // Changed from totalItems to total
        ...
        summary: {
          ...
          totalAmount: filteredQuotes.reduce(
      ...
      console.warn(
        "Failed to load real quotes data, falling back to service:",
        realDataError
      );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const quoteService = QuoteServiceFactory.getInstance();
    const { customerId, customerName, items, taxRate, currency, terms, notes } =
      validation.data;
