 ...
 */
export function transformCustomersToBootstrap(
  ...
  const parentCompanies = customers.filter(
    (customer) => !customer.parentCompanyId
  );
  ...
  return parentCompanies.map((parentCompany) => {
    ...
    const branchesArray = [
      // Parent company as main branch
      {
        id: parentCompany.id,
        displayName:
          parentCompany.addresses.find((addr) => addr.isPrimary)
      ...
      ...branches.map((branch) => ({
        id: branch.id,
        displayName:
          branch.name.replace(parentCompany.name + " - ", "") ||
        ...
        branchNumber: (branch as { branchNumber?: string }).branchNumber,
      })),
    ];
 ...
 */
export function transformProductsToCatalog(productsData: {
...
export function generateQuotesFromRealData(
  ...
  customers.forEach((customer, customerIndex) => {
    ...
    for (let i = 0; i < numQuotes; i++) {
      const quoteNumber = `QUO-${new Date().getFullYear()}-${String(
        customerIndex + 1
      ...
      const availableProducts = products.filter((product) => {
        const customPricing = customer.customProductPricing.find(
          (cpp) => cpp.productId === product.id
        );
      ...
      const selectedProducts = shuffleArray([...availableProducts]).slice(
        0,
        numLineItems
      );
      ...
      const lineItems = selectedProducts.map((product, lineIndex) => {
        ...
        const unitPrice =
          customer.type === "Retail" ? pricing.retail : pricing.consumer;

        const lineTotal = quantity * unitPrice;
        const discountPercent =
          Math.random() > 0.8 ? Math.floor(Math.random() * 15) + 5 : 0;
        const discountAmount = (discountPercent / 100) * lineTotal;
        ...
        return {
          ...
          totalPrice: lineTotal - discountAmount, // Backward compatibility
          taxable: true,
          notes: "",
      ...
      const discountAmount = lineItems.reduce(
        (sum, item) =>
          sum + (item.discount / 100) * item.quantity * item.unitPrice,
        0
      );
      ...
      if (daysSinceCreated > 60) {
        status =
          Math.random() > 0.7 ? QuoteStatus.Archived : QuoteStatus.Rejected;
      } else if (daysSinceCreated > 30) {
        status =
          Math.random() > 0.5 ? QuoteStatus.Pending : QuoteStatus.Approved;
