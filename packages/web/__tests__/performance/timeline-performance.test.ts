...
class MockTimelineApiClient {
  ...
  async getTimeline(quoteId: string): Promise<any> {
  ...
  async createEvent(quoteId: string, eventData: any): Promise<any> {
...
describe('Timeline API Performance Tests', () => {
  ...
  describe('Load Testing', () => {
    it('should handle concurrent requests within performance limits', async () => {
      ...
      console.log(`✓ Concurrent requests: avg=${avgDuration.toFixed(2)}ms, max=${maxDuration.toFixed(2)}ms`);
    });
  });
